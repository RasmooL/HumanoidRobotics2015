...
vector<Rect> getRectangles(Mat imgOrg, vector<vector<Point> > contours)
{
    //find rectangles
...
*/
    ...
    if(SHOW_RECTANGLES == ON)
    {
        ...
        {
            rectangle(imgOrg, boundRect[i].tl(), boundRect[i].br(), contourColor, 1, 8, 0);
        }
        imshow("imgRect", imgOrg);
    }
...
//        imshow("threshold", imgThreshold);
    ...
    if(contours.size() > 0)
    {
        ...
        vector<Rect> rectangles = getRectangles(imgOriginal, contours);

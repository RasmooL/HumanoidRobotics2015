...
[left_targets, right_targets] = load_msr('../../MSRDaily/a01_s01_e01_skeleton.txt');
...
for i = 1:nframes
    ...
    tpos = [];
    for j = 1:ninter
        tpos(:, j) = linterp_target(j/ninter, target) + [0; 0.2; 0];
    end
    ...
    %toc
    draw_chain(@leftarmnormalized, lsol);
    ...
    %toc
    draw_chain(@rightarmnormalized, rsol);
